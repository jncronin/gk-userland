...
cmake $CMAKE_OPTS -S openal-soft-1.24.2 -B build/openal -DALSOFT_BACKEND_SDL2=ON -DALSOFT_UTILS=OFF -DALSOFT_EXAMPLES=OFF -DALSOFT_INSTALL=OFF -DLIBTYPE=STATIC
...
mkdir -p build/glib
cd build/glib
../../glib-2.20.5/configure --host=arm-none-gkos --enable-static --disable-shared --prefix=$SYSROOT/usr
make -j 16 install
cd ../..

cmake $CMAKE_OPTS -S fluidsynth-2.4.2 -B build/fluidsynth -Denable-network=OFF
make -C build/fluidsynth -j16 install
