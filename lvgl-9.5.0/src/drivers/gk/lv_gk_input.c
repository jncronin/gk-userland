...
static uint32_t gk_key_to_lv(unsigned short gkkey);

/* Simple FIFO ring buffer, not thread safe */
#define GK_RB_SIZE 64
struct gk_rb
{
...
void gk_kbd_read_cb(lv_indev_t *indev, lv_indev_data_t *data)
{
    ...
    lv_memcpy(data, &cur_kbd, sizeof(lv_indev_data_t));
}
...
void gk_update_state()
{
    ...
    {
        lv_indev_data_t d;
        lv_memset(&d, 0, sizeof(lv_indev_data_t));
...
uint32_t gk_key_to_lv(unsigned short gkkey)
{
...
static int gk_rb_push(struct gk_rb *rb, const lv_indev_data_t *d)
{
    ...
    lv_memcpy(&rb->d[rb->wp], d, sizeof(lv_indev_data_t));
    rb->wp = new_wp;
    return 1;
}
...
static int gk_rb_pop(struct gk_rb *rb, lv_indev_data_t *d)
{
    ...
    lv_memcpy(d, &rb->d[rb->rp], sizeof(lv_indev_data_t));
