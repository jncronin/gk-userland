...
#elif defined(__unix__) && !defined(__GAMEKID__)
    #include <dlfcn.h>
...
/* statically linked, nothing to do */
#else
...
#elif defined(__GAMEKID__)
static void OS_GetExeName(char *buf, const unsigned maxpath) {
    /* TODO: get from a syscall */
    strncpy(buf, "dummy", maxpath);
}
#else
...
#if defined(__GNUC__) && !defined(_WIN32) && !defined(__GAMEKID__)
